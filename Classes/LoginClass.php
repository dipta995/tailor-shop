...
class LoginClass extends DB
{
    ...
    public function insertUser($data){
        ...
        if (empty($name) || empty($email) || empty($phone) || empty($password) || empty($role)) {
            ...
            $txt = "<div style='color:red; font-size: 15px;'>Only alphabets and whitespace are allowed For name.</div>";
            return $txt;
        }elseif (mysqli_num_rows($res)>0){
            $txt = "<div style='color:red; font-size: 15px;'>This Email Already been Registered </div>";
            return $txt;
        }elseif (mysqli_num_rows($res)>0){
            $txt = "<div style='color:red; font-size: 15px;'>This Mobile Number Already been Registered </div>";
            return $txt;
        }elseif ( strlen ($phone) != 9) {  
            $txt = "<div style='color:green; font-size: 15px;'>Mobile no. must have 9 digits.</div>";
            return $txt;
        }elseif ( strlen ($password) < 6) {  
            $txt = "<div style='color:green; font-size: 15px;'>Password must have 6 digits.</div>";
            return $txt;
        }else{
    ...
    public function updatePassword($data, $id) {
        ...
        if (empty($password) || empty($password1)) {
            return "<div style = 'color:red';>Field Must not be empty</div>";  
        }elseif ( strlen ($password) < 6) {  
            return "<div style = 'color:red';>Password must have 6 digits.</div>";  
            ...
            $result = $this->conn->query($query);
            if ($result) {
                echo $txt = "<div class='alert alert-success'>Password Updated. </div>";
            }
        }else{
